...
//       const element = arr5toPseudo[i].
// replace(regex, 'zero');
// заменяем 0 на зиро и склад в массив резулАрр
...
// получается NodeList 0,1,2 - инд., length=3,
//     for (let i = 0; i < paragrap.length; i++) {  // пробежимся циклом по параграфам
//       paragrap[i].addEventListener('click', function (e) {
// передаем функ котор вызовется по клику
//             let target = e.target; // click по конкретному параграфу
//             for (let i = 0; i < paragrap.length; i++) {
// пробегаемся в цикле чтобы сбросить с параграфов все цвета
//               paragrap[i].style.backgroundColor = '';
//             }
//             target.style.backgroundColor = colors[counter];
// colors[0], colors[1] добавляем цвет на параграф
...
// if (Object.values(item).includes(str))
// { // проверка на вхождение заданной строки
// с элементами массива Object.values(item)
// countriesArr.push(Object.values(item));
// True - в масстив countriesArr добавляем элемент
// ( весь массив Object.values(item))
// }
// });
...
// // ЗАДАЧА
// // Напишите функция deepEqual,
// которая сможет сравнивать 2
// объекта с разными уровнями вложенности.Напимер:
...
// sort((oneStudent, twoStudent) =>
// new Student(oneStudent).
// course - new Student(twoStudent).
// course).map((currentValue) => `${new User(currentValue).
//   fullName} - ${new Student(currentValue).
//     courseName}, ${new Student(currentValue).
//       course} курс`);
// // //   }
// // }
